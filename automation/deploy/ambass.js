...
function build() {
    return new Promise((resolve, reject) => {
      ...
      shell.exec(stackBuildCmd, afterStackBuildCmd);
    })
}

function afterStackBuildCmd(code, stdout, stderr) {
...
function correctBuild(err){
  return new Promise(resolve, reject => {
    ...
    if(err.match(/stack solver/gi)){
      ...
      shell.exec('stack solver --system-ghc --update-config',
          (code, solverOut, solverErr) => afterStackSolver(code, solverOut, solverErr, err));
...
function afterStackSolver(code, solverOut, solverErr, err){
